...
* 다른 프로토콜에 제공하는 서비스
    ...
    * IP : 주소지정, 전달, 데이터그램 패키징, 단편화와 재조합

    * TCP, UDP : 데이터 캡슐화, 장기 간 연결 관리

* 최종 사용자 서비스

    * 웹 애플리케이션을 위한 HTTP 프로토콜 등을 제공한다.

## TCP/IP 클라이언트/서버 구조 모델 

    TCP/IP 서비스는 주로 클라이언트/서버 모델에서 동작한다.

클라이언트는 서버에게 데이터, 기타 정보를 요청하는 메시지를 보내 통신을 시작한다.

서버는 메시지에 응답해 요청한 데이터, 오류 메시지 등의 정보를 제공한다.
...
### 트랜잭션 역할
    ...
    트랜잭션 : 특정 거래를 위해 컴퓨터 시스템이 완료해야 하는 일련의 처리 동작
...
TCP/IP 네트워크의 경우 네트워크 인터페이스 계층에서 동작하는 TCP/IP 프로토콜은 존재하지 않는다.
...
IP, 인터넷 제어 메시지 프로토콜(ICMP)과 라우팅 프로토콜(RIP,OSFP,BGP)등이 존재한다.

### 애플리케이션 계층

TCP/IP 모델의 최상위 계층이다

HTTP, FTP SMTP와 같은 애플리케이션 프로토콜부터 단순 네트워크 관리 프로토콜(SNMP), DHCP, DNS와 같은 프로토콜이 있다.
